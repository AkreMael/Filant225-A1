...
const MyQRCodeScreen: React.FC<MyQRCodeScreenProps> = ({ user, onBack, onTriggerPayment }) => {
  ...
  const currentStatus = qrData?.status || "En attente paiement frais (310 FCFA)";
  const isActive = currentStatus === "Code QR Actif";
  
  const getStepNumber = () => {
      if (currentStatus.includes("310")) return 1;
      if (currentStatus.includes("7 100")) return 2;
      if (isActive) return 3;
      if (currentStatus.includes("500")) return 4;
      return 1;
  };
      ...
      <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="bg-white rounded-[2.5rem] p-6 shadow-xl border border-gray-100 mb-8">
              <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase">Étape {step}/3</div>
              </div>
              
              <div className="relative flex justify-between items-center px-4">
                  <div className="absolute left-8 right-8 h-1 bg-gray-100 top-1/2 -translate-y-1/2 z-0">
                      <div className="h-full bg-orange-500 transition-all duration-1000" style={{ width: `${(step - 1) * 50}%` }}></div>
                  </div>
                  {[1, 2, 3].map((s) => (
                      <div key={s} className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center border-4 transition-all duration-500 ${step >= s ? 'bg-orange-500 border-orange-100 text-white scale-110 shadow-lg' : 'bg-white border-gray-50 text-gray-300'}`}>
              ...
              <div className="mt-10 w-full">
                  {!isActive ? (
                      <div className="space-y-4">
                          ...
                          >
                            {step === 1 ? 'Payer Frais Dossier (310 CFA)' : 'Activer mon QR Code (7 100 CFA)'}
